...
def test_prompt_contains_tightness_instruction():
    ...
    assert (
        "whole image" in p or "entire image" in p
...
def test_prompt_contains_per_object_instruction():
    ...
    p = _PROMPT.lower()
    assert (
        "per object" in p or "per instance" in p or "one bbox per" in p
...
def test_prompt_contains_few_shot_example():
    ...
    assert (
        has_good_bad or has_valid_invalid
...
def test_describe_photo_warns_on_whole_image_bbox(tmp_path, valid_jpeg_bytes, monkeypatch, caplog):
    ...
    assert any(
        "whole_image_bbox" in m and "photo_id=49" in m and "Stapler" in m for m in messages
    ), f"expected whole_image_bbox WARN with photo_id + name, got: {messages}"


def test_describe_photo_does_not_warn_on_tight_bbox(
    tmp_path, valid_jpeg_bytes, monkeypatch, caplog
):
    captured: dict = {}
    _patch_openai(
        monkeypatch,
