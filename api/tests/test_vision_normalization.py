...
def test_describe_photo_normalizes_pixel_space_bbox(tmp_path, monkeypatch, caplog):
    ...
    _patch_openai(monkeypatch, json.dumps({"suggestions": [
        {"name": "lamp base", "category": "tool", "confidence": 0.9,
         "bbox": [86, 138, 991, 997]},
    ]}))

    with caplog.at_level(logging.INFO, logger="app.services.vision"):
        hits, _ = vision_mod.describe_photo(
            photo_path, "http://fake/v1", "k", "qwen3p6-plus",
            max_px=2048, photo_id=1,
        )
...
def test_describe_photo_normalizes_non_square_image(tmp_path, monkeypatch):
    ...
    hits, _ = vision_mod.describe_photo(
        photo_path, "http://fake/v1", "k", "qwen3p6-plus",
        max_px=2048, photo_id=2,
    )

    bbox = hits[0]["bbox"]
    assert bbox == pytest.approx(
        [100 / 1024, 400 / 768, 800 / 1024, 600 / 768], abs=0.001
    )
...
def test_whole_image_warn_uses_normalized_coverage(tmp_path, monkeypatch, caplog):
    ...
    assert "coverage=0." in whole_msgs[0], f"coverage must be a decimal fraction, got: {whole_msgs[0]}"
...
def test_prompt_contains_empty_list_rejection_clause():
    ...
    assert "suggestions: []" in p or "suggestions\":[]" in p or "empty list" in p, (
