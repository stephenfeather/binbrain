...
class TestPatchLocationAPI:
    def test_rename_location(self, client, db):
        _cleanup(db)
        loc_id = client.post("/locations", json={"name": "Garage"}).json()["location"][
            "location_id"
        ]
        ...
        names = [loc["name"] for loc in client.get("/locations").json()["locations"]]
    ...
    def test_empty_patch_is_noop_200(self, client, db):
        ...
        _cleanup(db)
        loc_id = client.post("/locations", json={"name": "Unchanged"}).json()["location"][
            "location_id"
        ]
    ...
    def test_rename_conflict_409(self, client, db):
        ...
        other_id = client.post("/locations", json={"name": "Pantry"}).json()["location"][
            "location_id"
        ]
    ...
    def test_non_admin_key_forbidden(self, client, user_client, db):
        ...
        _cleanup(db)
        loc_id = client.post("/locations", json={"name": "Admin-only"}).json()["location"][
            "location_id"
        ]
