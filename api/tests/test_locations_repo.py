...
class TestLocationRepository:
    ...
    def test_soft_delete_location_nulls_bin_references(self, db):
        ...
        db.commit()
        db.execute(
            text("INSERT INTO bins (bin_id, location_id) VALUES ('BIN-DEL-TEST', :loc_id)"),
        ...
        db.commit()
        row = db.execute(
            text("SELECT location_id FROM bins WHERE bin_id = 'BIN-DEL-TEST'")
        ).scalar()
        assert row is None
    ...
    def test_update_bin_location(self, db):
        ...
        db.execute(
            text("INSERT INTO bins (bin_id) VALUES ('BIN-LOC-TEST') ON CONFLICT DO NOTHING"), {}
        )
        db.commit()
        ...
        row = db.execute(
            text("SELECT location_id FROM bins WHERE bin_id = 'BIN-LOC-TEST'")
        ).scalar()
    ...
    def test_update_bin_location_clear(self, db):
        ...
        db.execute(
            text("INSERT INTO bins (bin_id, location_id) VALUES ('BIN-CLR-TEST', :loc_id)"),
            {"loc_id": loc["location_id"]},
        )
        db.commit()
        ...
        assert updated is True
        row = db.execute(
            text("SELECT location_id FROM bins WHERE bin_id = 'BIN-CLR-TEST'")
        ).scalar()
        assert row is None
